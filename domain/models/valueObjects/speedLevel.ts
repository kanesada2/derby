...
export class SpeedLevel extends EventEmitter {
    ...
    private _max: number = SpeedLevel.MAX_BASE;
    ...
    private determineParameters(): void {
        ...
        this._max = SpeedLevel.MAX_BASE + SpeedLevel.MAX_AMPLIFIER_MAX * lotList[lotCountBase - 1];
        this._motivatingMin = this._max - SpeedLevel.UNIT;
    ...
    isPleasant(): boolean {
        return this._pleasantMin <= this._current && this._current <= this._pleasantMax;
    }

    isMotivating(): boolean {
        return this._motivatingMin <= this._current;
    }
    ...
    decreaseClock(): void {
        ...
        this._current = Math.max(this._current - SpeedLevel.DECREASE_SPAN, SpeedLevel.MIN);
    ...
    increaseByCrawl(): void {
        ...
        this._current = Math.min(this._current + SpeedLevel.INCREASE_SPAN, this._max);
        this.emit('change', this._current);
    }

    fixByExhausted(modifier: number): void {
        const amplifier = (SpeedLevel.MIN - SpeedLevel.EXHAUSTED_BASE) * modifier;
        this._current = SpeedLevel.EXHAUSTED_BASE + amplifier;
